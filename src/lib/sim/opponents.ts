...
import { computeBaseSpeed, STYLE_NUM, type UmaSimState } from "./types";
import { initialHp, postStartMinSpeed } from "./physics";
...
export function generateOpponent(
  ...
  uma.hp = uma.hpMax;
  // Start at the post-start minimum (matches what the runner does for the
  // player). The start-dash bonus accelerates them up to base speed quickly.
  uma.velocity = postStartMinSpeed(uma, computeBaseSpeed(distance));
  return uma;
}
